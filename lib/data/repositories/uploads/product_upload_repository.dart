...
class ProductUploadsRepository extends GetxController {
  ...
  // Function to fetch user details based on user ID
  Future<Map<String, String>> fetchBrandListWithGivenBrandIdList(Set<String> categoryIds) async {
    try {
      if(categoryIds.isEmpty) return <String, String>{};
      final documentSnapshot = await _db
          .collection("brands")
          .where('Id', whereIn: categoryIds.toList())
          .get();
      var map = <String, String>{};
      for(var doc in documentSnapshot.docs) {
        final brand = BrandModel.fromSnapshot(doc);
  ...
  // Function to fetch user details based on user ID
  Future<Set<String>> fetchCategoryBrandListWithGivenCategoryId(String id) async {
    try {
      final documentSnapshot = await _db
      ...
      var set = <String>{};
      for(var doc in documentSnapshot.docs) {
        final brand = BrandCategoryModel.fromSnapshot(doc);
